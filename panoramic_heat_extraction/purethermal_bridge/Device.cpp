...
Device::Frame Device::GetFrame()
{
	...
	_deviceCondition.wait_for(lock, std::chrono::seconds(5), [this]() { return !_frameRequested; });
	return _frameBuffer;
}
...

	{
		...
		if (SUCCEEDED(hr) && sample)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				// Always copy frame data (valid even during FFC)
				CopyMemory(_frameBuffer.data.data(), data, _frameBuffer.data.size() * sizeof(std::uint16_t));
				
				// Extract telemetry (skip if FFC in progress)
				ExtractTelemetryData(reinterpret_cast<std::uint16_t*>(data) + _frameBuffer.data.size());
